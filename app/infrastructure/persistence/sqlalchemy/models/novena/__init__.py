...
class NovenaDay(Base):
    ...
    # Relationships
    sections = relationship("NovenaDaySection", back_populates="day", cascade="all, delete-orphan")
    user_progress = relationship("UserNovenaProgress", back_populates="day", cascade="all, delete-orphan")
...
class NovenaDaySection(Base):
    ...
    __table_args__ = (
        UniqueConstraint("day_id", "section_type", "position", name="novena_day_sections_day_id_section_type_position_key"),
        {"schema": "parranda"}
    )
    
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    day_id = Column(UUID(as_uuid=True), ForeignKey("parranda.novena_days.id"), nullable=False)
...
class UserNovenaProgress(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("parranda.users.id"), primary_key=True)
    day_id = Column(UUID(as_uuid=True), ForeignKey("parranda.novena_days.id"), primary_key=True)
