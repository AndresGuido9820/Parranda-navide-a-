...
from app.application.dtos.auth import LoginRequest, AuthResponse, UserResponse
...
class LoginUserUseCase:
    ...
    def execute(self, request: LoginRequest) -> AuthResponse:
        ...
        access_token, refresh_token = self.jwt_service.create_token_pair(data=token_data)
        
        # Create session
        session_token_hash = self.jwt_service.generate_session_token()
        expires_at = datetime.now(timezone.utc) + timedelta(days=30)  # 30 days for refresh token
